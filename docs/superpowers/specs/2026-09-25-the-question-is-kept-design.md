...
`foreign` and `bad_request` write nothing: the first is not a visitor of the page and the second sent no question the shape accepts. A stream the visitor closed before the answer ended writes the line with what the loop had gathered, since the question was asked. The line is written once the answer is complete or the refusal is decided, with the stream's end or the refusal's send and never before the loop is done, so a line's presence in the log means the visitor got their answer or their refusal.
...
The package's `bin/deploy.mjs` gains a `report` command. Under the analyst's credentials it lists the view's entries of one ISO week, the week that holds the day before the run or the week named on the command line, from its Monday at midnight UTC to the next, through the Logging API, with the auth library the Firestore client already brings and no further dependency, and writes one Markdown file, `YYYY-Www.md` by that week, to the bucket, so two runs write the same file and a week can be rebuilt while its lines are kept: how many questions, how many answered, by language, the entities most cited, and the unanswered questions in full, each with its `empty` and its `refused`. It prints the counts and never a question, because the run's log in a public repository is public.
