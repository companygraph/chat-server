...
7. **The report covers one ISO week, its Monday at midnight UTC to the next**: the week of the day before `now`, so a run on Monday at whatever hour names and covers the week that ended on Sunday, or the week named on the command line as `YYYY-Www`, checked before anything is read. Two runs write the same file, and a week is rebuilt while its lines are kept. The report is written even when the week held no question. This replaced a first ruling, seven days back from `now`, after review: that window's name did not match its contents, and a schedule's drift made weeks overlap or gap.
