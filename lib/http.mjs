...
//
// An error that is not a refusal is logged as its name, its status and its code and never as the
// object: a provider's error carries its own message about the request, which can hold the words
// the visitor typed, and the privacy page says not a word of them is kept.
...
export function createHttpServer({ config, host, model, meter, bucket }, { pageCss = null, pageBrand = null, pageIcon = null } = {}) {
  ...
  return http.createServer(async (req, res) => {
    ...
    try {
      ...
      try {
        await answer({ host, model, meter, questionCap: config.questionIndexChars }, { messages: body.messages, lang: body.lang, signal: ac.signal }, (event, data) => stream.send(event, data));
      } catch (err) {
        if (!stream.opened) throw err;
        if (err instanceof ChatError) { stream.send("error", refusal(err)); }
        else { logInternal(err); stream.send("error", { error: { code: "internal", message: "internal error" } }); }
      }
      stream.end();
    } catch (err) {
