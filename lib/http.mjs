...
import { MODEL, credentialFault } from "./model.mjs";
...
// A credential's fault is the one exception and adds its sentence as `detail`: the sentence is
// this package's or the SDK's about the token and holds nothing the visitor sent, and on the day
// a deployment switches it is what says which side refused.
const logInternal = (err) => {
  const cred = credentialFault(err);
  console.error(line("chat.error", "ERROR", { kind: "internal", message: "internal error while answering", name: err?.name ?? null, status: err?.status ?? cred?.statusCode ?? null, code: err?.code ?? null, ...(cred ? { detail: cred.message } : {}) }));
};
