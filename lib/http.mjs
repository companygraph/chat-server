...
const logInternal = (err) => console.error("chat: internal", err?.name, err?.status ?? "", err?.code ?? "");
...
// Two keys Cloud Logging lifts out of a JSON line on standard output: the severity becomes the
// entry's own, and the labels become the entry's labels, so a reader filters the kept lines in
// the console as labels.logger="chat.question", the way a logger's name is filtered elsewhere.
// The sink does not need them; it reads kind from the payload.
const LINE_HEAD = { severity: "INFO", "logging.googleapis.com/labels": { logger: "chat.question" } };
...
export function createHttpServer({ config, host, model, meter, bucket, log = console.log }, { pageCss = null, pageBrand = null, pageIcon = null } = {}) {
  ...
  return http.createServer(async (req, res) => {
    ...
    const keep = (signals, refused) => {
      ...
      log(JSON.stringify({ ...LINE_HEAD, kind: "question", question, lang, cited: s.cited, calls: s.calls, empty: s.empty, rounds: s.rounds, refused }));
    };
    try {
