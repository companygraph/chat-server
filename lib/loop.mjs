...
export async function answer({ host, model, meter, questionCap = 4000 }, { messages, lang, signal }, emit) {
  const turns = window(validateMessages(messages));
  let spent = 0;
  const conv = turns.map((t) => ({ role: t.role, content: t.content }));
  const system = systemPrompt(host.instructions, lang, await host.types(), await host.questions(), questionCap);
  const cited = new Set();
  const named = new Set();
  let settled;
  let last;
