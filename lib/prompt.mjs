...
// rather than reading as the whole of what the model answers.
const QUESTION_OVERFLOW = "; and more, found by search with type question";
const QUESTION_PREFIX = "Questions this model answers, each an entity of type question: ";
...
// questions at all, rather than a line of punctuation with nothing in it.
export function questionIndexLine(titles = [], cap = 4000) {
  if (!titles.length) return "";
  const quote = (t) => `"${t.replaceAll('"', '\\"')}"`;
  let body = "";
  let n = 0;
  for (; n < titles.length; n++) {
    const piece = (n ? "; " : "") + quote(titles[n]);
    const candidate = body + piece;
    if ((QUESTION_PREFIX + candidate + QUESTION_OVERFLOW).length > cap) break;
    body = candidate;
  }
  if (!body) return "";
  return QUESTION_PREFIX + body + (n === titles.length ? "." : QUESTION_OVERFLOW);
}
...
export function systemPrompt(instructions, lang, types = [], questions = [], questionCap = 4000) {
  ...
  const questionsLine = hasQuestionType ? questionIndexLine(questions, questionCap) : "";
