...
// listTools, list_types, and up to QUESTION_PAGE_LIMIT pages of question titles — and /questions
// is deliberately asked of the host on every page view, uncounted by the message bucket, so
// retrying a degraded host on every single call could cost one reconnect a page view. This holds
// the next attempt off for a short while instead, thirty seconds being long enough that a burst
// of page views during an outage costs a handful of attempts and not one each, and short enough
// that a host's own recovery is found soon after it happens.
