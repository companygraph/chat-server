...
// One line under chat.index, as lib/log.mjs writes every line. Fired at most once a fetch, never
// once a request — the fetch happens on a commit move, the prompt's own line is built fresh
// every message from whatever that fetch last cached.
const logTitleTooLong = (cap) => console.error(line("chat.index", "WARNING", { kind: "question-index", message: `a title alone is longer than the CHAT_QUESTION_INDEX_CHARS cap (${cap}) and is left out`, cap }));
